...
func InitView(ctx context.Context, req InitViewRequest) error {
	if req.Name == "" {
	...
	entrypoint, err := createViewEntrypoint(slug, req.Name)
	if err != nil {
		return err
	}
	cwd, err := os.Getwd()
...
func createViewEntrypoint(slug string, name string) (string, error) {
