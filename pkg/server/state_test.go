...
func TestStore(t *testing.T) {
	...
	task2 := "task2"
	runID2 := "task2_run"
	run2 := LocalRun{RunID: runID2, TaskID: task2, Status: api.RunSucceeded}
...
func TestStoreUpdate(t *testing.T) {
	...
	taskID := "task1"
	runID := "task1_run"
	run := LocalRun{RunID: runID, TaskID: taskID, Status: api.RunQueued}
	...
	now := time.Now()
	updatedRun := LocalRun{RunID: runID, TaskID: taskID, Status: api.RunFailed, FailedAt: &now}
	_, err := store.update(runID, func(run *LocalRun) error {
		*run = updatedRun
		return nil
	})
